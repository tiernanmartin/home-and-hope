...
make_owner_public_categories <- function(...){
  ...
  loadd(parcel_ready, suitability, owner_name_full, suitability_tax_exempt, owner_antijoin_names, owner_name_category_key)
  
pub <- suitability_tax_exempt %>% 
  full_join(owner_name_full, by = "PIN") %>% 
  filter(SUIT_OWNER_TAX_E) %>% 
  gather(CATEGORY, VALUE, SUIT_OWNER_PUBLIC:SUIT_OWNER_OTHER_EXEMPT) %>% 
  filter(VALUE) %>% 
  transmute(PIN,
            OWNER = OWNER_NAME_FULL,
            CATEGORY = str_replace(CATEGORY,"SUIT_OWNER_",""))

# Check out the top 5 from each tax exempt type:
# pub %>% 
